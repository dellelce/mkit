...
build_sanity_gnuconf()
{
  typeset acpath=""
  [ -z "$1" ] && {
    echo "build_sanity_gnuconf srcdirectory"
    return 1
  }
  [ ! -d "$1" ] && {
    echo "build_sanity_gnuconf: invalid srcdirectory: $1"
    return 1
  }
  ...
  [ ! -e "$1/configure" -a -e "$1/configure.ac" -a -e "$1/autogen.sh" ] &&
    {
      ...
      cd "$cwd"
      [ $ar_rc -ne 0 ] && {
        echo "autogen.sh failed with rc = $ar_rc"
        return $ar_rc
      }
  ...
  [ ! -e "$1/configure" -a -e "$1/configure.ac" ] &&
    {
      ...
      cd "$cwd"
      [ $ar_rc -ne 0 ] && {
        echo "autoreconf failed with rc = $ar_rc"
        return $ar_rc
      }
      build_sanity_gnuconf $1
      return $?
  ...
  [ ! -f "$1/configure" ] && {
    echo "build_sanity_gnuconf: no configure file in: $1"
    return 1
  }
...
build_gnuconf()
{
  ...
  [ -z "$dir" ] && {
    echo "usage: build_gnuconf id source_dir_path"
    return 1
  }
  ...
  build_sanity_gnuconf $dir >${logFile} 2>&1
  rc=$?

  [ $rc -ne 0 ] && {
    cat "${logFile}"
    echo "build_gnuconf: build sanity tests failed for $dir"
    return $rc
  }
...
add_run_dep()
{
  ...
  [ -z "$RUNTIME_LIST" ] && {
    export RUNTIME_LIST="$id_list"
  } || {
    export RUNTIME_LIST="$RUNTIME_LIST $id_list"
  }
...
add_build_dep()
{
  ...
  [ -z "$id_list" ] && return 1 # sanitycheck
  [ -z "$BUILDTIME_LIST" ] && {
    export BUILDTIME_LIST="$id_list"
  } || {
    export BUILDTIME_LIST="$BUILDTIME_LIST $id_list"
  }

  # remove possible duplicates
  BUILDTIME_LIST=$(
    for item in $BUILDTIME_LIST; do
      echo $item
    done | awk '!x[$0]++'
  )
}
...
run_build()
{
  ...
  [ ! -z "$LDFLAGS" ] && {
    export BASE_LDFLAGS="$CFLAGS"
    unset LDFLAGS
  }

  # download latest archives / builds name mapping
  download || {
    echo "Download failed for one of the components"
    exit 1
  }
...
'
  ...
  # Main Build Loop!
  for pkg in $BUILDTIME_LIST $RUNTIME_LIST; do
    build=0
    [ ${pkg} != "${pkg#_}" ] && {
      build=1
      pkg="${pkg#_}"
    ...
    [ "$build" -eq 1 ] &&
      {
        prefix="$buildprefix" $func $options
        rc=$?
      } ||
      {
        # Launch Build
...
build_raw_lite()
{
  ...
  for makedir in $pkgbuilddir $dir; do
    [ -f "$makedir/Makefile" ] && break
  done

  [ -z "$makedir" ] && {
    echo "Failed to find a Makefile."
    return 1
  }
...
build_perlmodule()
{
  ...
  PERL5LIB=$prefix/share/perl5 \
    $prefix/bin/perl Makefile.PL PREFIX=$prefix >${logFile} 2>&1
  rc_configurepl="$?"
