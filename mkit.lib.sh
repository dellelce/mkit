  ...
  for pkg in $RUNTIME_LIST; do
    ...
    [ -f "$custom_download" ] &&
      ...
      } ||
      {
        ...
        fn="$PWD/$fn"
        [ ! -f "$fn" ] && {
          echo "Failed downloading $pkg: rc = $srcget_rc"
          return $srcget_rc
        }
      }
...
un_xz()
{
  ...
  [ ! -f "$fn" ] && {
    echo "uncompress: $fn is not a file."
    return 1
  }
...
do_uncompress()
{
  ...
  eval "fn=\$fn_$id"
  uncompress $id $fn || {
    echo "Failed uncompress for: $fn_$id"
    return 1
  }
